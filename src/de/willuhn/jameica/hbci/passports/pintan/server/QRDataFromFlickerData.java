 ...
 *
 */
public class QRDataFromFlickerData
{
   ...
   * @throws IOException
   */
  public static void MakeQRDataFromFlickerData(StringBuffer retData) throws IOException
  {
    byte[] data = DatatypeConverter.parseHexBinary(retData.toString());
    ...
    String buf = new String("image/png");
    int len = buf.length();
    ...
    retData.append(sb.toString());

    // Logger.debug("Data for NEED_PT_QRTAN: " + javax.xml.bind.DatatypeConverter.printHexBinary(stringBuffer.toString().getBytes(Comm.ENCODING)));
  }
